...
import { IPendingUserModel } from "../../pendingUser/pendingUser.interface";
...
const requestOTP = async (userId: number) => {
    ...
    if (!dbPendingUser) {
        ...
        dbPendingUser = await PendingUserService.save({
            ...
            otp: "",
            attempt_count: 0,
        } as IPendingUserModel);

    }

    const now = Date.now();
    const requestedAt = new Date(dbPendingUser.requested_at).getTime();
    const timeGap = now - requestedAt;

    // Check if the cooldown period has passed
    if (timeGap < constants.SESSIONS.COOLDOWN_TIME) {
        throw new ForbiddenError("Please wait before making another attempt!");
    }
    ...
    if (dbPendingUser.attempt_count > constants.SESSIONS.MAXIMUM_RETRY) {
        if (timeGap < constants.SESSIONS.BLOCKED_COOL_DOWN_TIME) {
            throw new ForbiddenError("You have exceeded the retry count. Please try again later!");
        } else {
    ...
    dbPendingUser.requested_at = new Date();
